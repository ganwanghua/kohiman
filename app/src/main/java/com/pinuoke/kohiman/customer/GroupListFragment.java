...
public class GroupListFragment extends BaseFragment implements OnRefreshLoadMoreListener {
    ...
    public GroupListFragment(int i, String is_followed, String status_id) {
    ...
    protected void initView() {
        ...
        if (i == 0 || i == 1) {
            groupCustomerList(page, is_followed);
        } else {
    ...
    private void groupCustomerList(int page, String s) {
        Map<String, String> map = new HashMap<>();
        map.put("s", "/sales/client.index/all");
        map.put("is_followed", s);
        map.put("page", page + "");
        map.put("token", FastData.getToken());
        dataRepository.groupCustomerList(map, new RemotDataSource.getCallback() {
            @Override
            public void onFailure(String info) {
